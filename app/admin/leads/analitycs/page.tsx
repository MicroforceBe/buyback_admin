...
function Bar({
  ...
  const width =
    total > 0
      ? Math.max(2, Math.round((value / total) * 100))
      : 0;
...
export default async function LeadsAnalyticsPage() {
  ...
  const voucher = leads.filter(
    (l) => l.wants_voucher
  ).length;

  const ship = leads.filter(
    (l) => l.delivery_method === "ship"
  ).length;

  const dropoff = leads.filter(
    (l) => l.delivery_method === "dropoff"
  ).length;

  const totalValueCents = leads.reduce((sum, l) => {
    const cents =
      l.wants_voucher &&
      typeof l.final_price_with_voucher_cents ===
        "number"
  ...
  const avgValueCents =
    total > 0
      ? Math.round(totalValueCents / total)
      : 0;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Overzicht op basis van maximaal de laatste
            5000 leads.
          </p>
        </div>
        ...
        <Link
          href="/admin/leads"
          className="bb-btn h-9 text-xs px-3"
        >
          ← Terug naar leads
        </Link>
      </div>
