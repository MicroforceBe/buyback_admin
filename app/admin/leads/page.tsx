...
export default async function LeadsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return (
    <div className="w-full p-4 space-y-4">
      ...
      <form className="p-3 border rounded-lg bg-white space-y-2" method="GET">
        {/* Globale (compact) */}
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-2">
          <input name="q" defaultValue={q} placeholder="Zoek overal…" className="bb-input h-10 px-3 py-2 text-sm w-full border rounded-md" />
          <div className="flex gap-2">
            <input type="date" name="from" defaultValue={from} className="bb-input h-10 px-3 py-2 text-sm border rounded-md w-full" />
            <input type="date" name="to" defaultValue={to} className="bb-input h-10 px-3 py-2 text-sm border rounded-md w-full" />
          </div>
          <select name="limit" defaultValue={String(limit)} className="bb-select h-10 px-3 py-2 text-sm border rounded-md">
            <option value="25">25 / p</option>
            <option value="50">50 / p</option>
            <option value="100">100 / p</option>
          </select>
          <div className="flex gap-2">
            <button className="bb-btn h-10 px-3" type="submit">Filteren</button>
            <Link href="/admin/leads" className="bb-btn h-10 px-3 subtle">Reset</Link>
          </div>
        </div>

        {/* We plaatsen HIER GEEN kolomfilters meer — die staan onderaan in de tabelkop (tweede rij). */}
        {/* De waarden worden alsnog vanuit searchParams overgenomen in die rij */}
      </form>

      {/* Tabel met tweerijige header (2e rij = kolomfilters) */}
      <div className="overflow-auto">
        <form method="GET">
          {/* behoud alle QS-waarden die niet door table-filters zelf worden gezet */}
          {Object.entries(qsBase).map(([k, v]) =>
            ["f_order","f_date_from","f_date_to","f_customer","f_model","f_variant","f_price_min","f_price_max","f_status"].includes(k)
              ? null
            ...
            <thead>
              ...
              <tr className="bg-white border-b border-gray-200">
                ...
                <th className="px-3 py-2 border-r border-gray-200">
                  <input
                    name="f_order"
                    defaultValue={f_order}
                    placeholder="BB…"
                    className="bb-input h-10 w-full px-2 text-sm border rounded-md"
                  />
                </th>
                {/* Datum (van/tot) */}
                <th className="px-3 py-2 border-r border-gray-200">
                  <div className="flex gap-2">
                    <input type="date" name="f_date_from" defaultValue={f_date_from} className="bb-input h-10 w-full px-2 text-sm border rounded-md" />
                    <input type="date" name="f_date_to" defaultValue={f_date_to} className="bb-input h-10 w-full px-2 text-sm border rounded-md" />
                  </div>
                </th>
                {/* Klant */}
                <th className="px-3 py-2 border-r border-gray-200">
                  <input
                    name="f_customer"
                    defaultValue={f_customer}
                    placeholder="naam/email/tel"
                    className="bb-input h-10 w-full px-2 text-sm border rounded-md"
                  />
                </th>
                {/* Model */}
                ...
                <th className="px-3 py-2 border-r border-gray-200">
                  <div className="flex gap-2">
                    <input name="f_price_min" defaultValue={f_price_min} placeholder="Min" className="bb-input h-10 w-full px-2 text-sm border rounded-md" inputMode="decimal" />
                    <input name="f_price_max" defaultValue={f_price_max} placeholder="Max" className="bb-input h-10 w-full px-2 text-sm border rounded-md" inputMode="decimal" />
                  </div>
                </th>
                {/* Status */}
                <th className="px-3 py-2">
                  <div className="flex items-center gap-2">
                    <select name="f_status" defaultValue={f_status} className="bb-select h-10 px-2 text-sm border rounded-md w-full">
                    ...
                    <button className="bb-btn h-10 px-3" type="submit">Toepassen</button>
                  </div>
                </th>
              </tr>
            </thead>
            ...
            <tbody>
              {(data ?? []).map((lead, idx) => (
                ...
                >
                  ...
                  {/* Prijs (editable met hogere input) */}
                  <td className="px-3 py-2 border-r border-gray-200 align-top">
                    ...
                    <form action={updateLeadInlineAction} className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        className="bb-input h-10 w-28"
                        inputMode="decimal"
                        placeholder="0.00"
                      />
                      <button className="bb-btn subtle h-10 px-3" type="submit" title="Opslaan">💾</button>
                    </form>
                  </td>

                  {/* Status (editable met hogere select) */}
                  <td className="px-3 py-2 align-top">
                    <form action={updateLeadInlineAction} className="flex items-center gap-2">
                      <input type="hidden" name="id" value={lead.id} />
                      <select name="status" defaultValue={lead.status ?? "new"} className="bb-select h-10">
                        <option value="new">Nieuw</option>
                        <option value="received_store">Ontvangen in winkel</option>
                        <option value="label_created">Verzendlabel aangemaakt</option>
                        <option value="shipment_received">Zending ontvangen</option>
