...
// Sorteer UI helpers
function sortIcon(col: string, current: string, dir: "asc" | "desc") {
  if (col !== current) return "↕";
  return dir === "asc" ? "↑" : "↓";
}

function thClass(col: string, current: string) {
  const base = "px-3 py-2 border-b border-r border-gray-200";
  const active = col === current ? " bg-yellow-50" : "";
  return base + active;
}
...
export default async function LeadsPage({
  ...
  const makeSortHref = (col: string) => {
    ...
    const nextDir = sortCol === col && dir === "asc" ? "desc" : "asc";
    sp.set("sort", col);
    sp.set("dir", nextDir);
    sp.set("page", "1");
    return `?${sp.toString()}`;
  ...
  // compacte filterinputs (iets hogere controls)
  const inputCls =
    "bb-input h-9 px-3 py-2 text-sm border rounded-md";
  const selectCls =
    "bb-select h-9 px-3 py-2 text-sm border rounded-md";
  const btnCls =
    "bb-btn h-9 px-3 inline-flex items-center justify-center border rounded-md";
    ...
    <div className="w-full p-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Link href={`/admin/leads/export?download=1`} className={btnCls}>
            ⬇︎Export CSV
          </Link>
      ...
      {/* Compacte filterbalk */}
      <form className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2 p-3 border rounded-lg bg-white" method="GET">
        ...
        <div className="flex gap-2">
          ...
          <Link href="/admin/leads" className={`${btnCls} subtle`}>
            Reset
          </Link>
        </div>
      </form>
        ...
        <table className="w-full text-sm border border-gray-200">
          <thead className="bg-gray-50">
            <tr className="text-left text-gray-700">
              <th className={thClass("order_code", sortCol)}>
                <a
                  href={makeSortHref("order_code")}
                  className="font-semibold hover:underline flex items-center gap-1"
                >
                  Order ID
                  <span className="text-gray-500">
                    {sortIcon("order_code", sortCol, dir)}
                  </span>
                </a>
              ...
              <th className={thClass("created_at", sortCol)}>
                <a
                ...
                >
                  Datum
                  <span className="text-gray-500">
                    {sortIcon("created_at", sortCol, dir)}
                  </span>
                </a>
              </th>
              <th className={thClass("first_name", sortCol)}>
                <a
                  href={makeSortHref("first_name")}
                  className="font-semibold hover:underline flex items-center gap-1"
                >
                  Klant
                  <span className="text-gray-500">
                    {sortIcon("first_name", sortCol, dir)}
                  </span>
                </a>
                <div className="text-[11px] text-gray-500">
                  klik om klantinfo te tonen
                </div>
              </th>
              <th className={thClass("model", sortCol)}>
                <a
                ...
                >
                  Model
                  <span className="text-gray-500">
                    {sortIcon("model", sortCol, dir)}
                  </span>
                </a>
              </th>
              <th className={thClass("capacity_gb", sortCol)}>
                <a
                  href={makeSortHref("capacity_gb")}
                  className="font-semibold hover:underline flex items-center gap-1"
                >
                  Variant
                  <span className="text-gray-500">
                    {sortIcon("capacity_gb", sortCol, dir)}
                  </span>
                </a>
              </th>
              <th className={thClass("final_price_cents", sortCol)}>
                <a
                ...
                >
                  Prijs (EUR)
                  <span className="text-gray-500">
                    {sortIcon("final_price_cents", sortCol, dir)}
                  </span>
                </a>
              </th>
              <th className={thClass("status", sortCol)}>
          ...
          <tbody>
            {(data ?? []).map((lead, idx) => (
              ...
              >
                ...
                <td className="px-3 py-2 border-r border-gray-200 align-top">
                  <details>
                    ...
                    <div className="mt-2 text-xs leading-5 space-y-2">
                      ...
                      <pre className="bg-white border border-gray-200 rounded p-2 overflow-auto max-h-[220px]">
                        {lead.answers ? JSON.stringify(lead.answers, null, 2) : "—"}
                      </pre>
                      <div className="pt-1">
                        <form action={deleteLeadAction}>
                          <input type="hidden" name="id" value={lead.id} />
                          <button className="bb-btn danger" type="submit">
                            Verwijderen
                          </button>
                        </form>
                      </div>
                ...
                {/* Variant — voorlopig capacity in GB */}
                <td className="px-3 py-2 border-r border-gray-200 align-top">
                  {lead.capacity_gb ? `${lead.capacity_gb} GB` : "—"}
                </td>
                ...
                <td className="px-3 py-2 border-r border-gray-200 align-top">
                  <form action={updateLeadInlineAction} className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      defaultValue={(((lead.final_price_cents ?? 0) / 100)).toFixed(2)}
                      className="bb-input h-9 px-2 text-sm w-28"
                      inputMode="decimal"
                      placeholder="0,00"
                    />
                    <button className="bb-btn subtle h-9 px-2" type="submit" title="Opslaan">
                      💾
                    </button>
                  </form>
                </td>
                ...
                <td className="px-3 py-2 align-top">
                  <form action={updateLeadInlineAction} className="flex items-center gap-2">
                    ...
                    <select
                      ...
                      className="bb-select h-9 px-2 text-sm"
                    >
