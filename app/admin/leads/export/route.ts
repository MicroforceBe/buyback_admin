...
export async function GET(req: Request) {
  ...
  for (const r of rows) {
    lines.push([
      ...
      escape(r.first_name ?? ''), escape(r.last_name ?? ''), escape(r.email ?? ''), escape(r.phone ?? ''),
      escape(r.delivery_method ?? ''), escape(r.shop_location ?? ''),
      escape(r.street ?? ''), escape(r.house_number ?? ''), escape(r.postal_code ?? ''), escape(r.city ?? ''), escape(r.country ?? ''),
      escape(maskIban(r.iban ?? '')),
      escape(r.status ?? ''), escape(r.admin_note ?? ''), escape(r.updated_at ?? ''),
  ...
  return new NextResponse(csv, {
    ...
    headers: {
      ...
      'content-disposition': 'attachment; filename="buyback-leads.csv"'
    }
  });
}
