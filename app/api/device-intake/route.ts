...
export async function POST(req: Request) {
  try {
    ...
    const capacityGb = storageToGb(body.storage);

    const suggestedArticle = await prisma.erpArticle.findFirst({
      where: {
        active: true,
        missing_from_erp: false,
        refurbished_product: true,

        brand: {
          equals: body.brand || "Apple",
          mode: "insensitive",
        },
        ...
        capacity_gb: capacityGb || undefined,

        condition_grade: body.cosmeticGrade || undefined,

        vat_margin:
          body.vatScheme === "margin"
            ? true
            : body.vatScheme === "vat"
              ? false
              : undefined,
    ...
    return NextResponse.json({
      ...
      previousSessions:
        existingDevice?.sessions.length || 0,
      ...
      suggestedSku: suggestedArticle?.sku || null,
      suggestedArticleTitle: suggestedArticle?.title || null,
      suggestedArticleId: suggestedArticle?.id || null,
